...
Companion docs:
- `docs/epics/002_password_reset_foundation/todo.md`
...
Use this section when the package is resumed after a pause or restart.
...
- If prior implementation stopped unexpectedly but the documented next action is unchanged and still safe, resume from that next action.
...
- Before stopping, sync `todo.md`, `docs/BLACKBOARD.md`, and any affected companion docs so the next contributor can resume from exact current truth.
