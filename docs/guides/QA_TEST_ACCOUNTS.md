...
Purpose: narrow-scope operating note for unattended/runtime QA accounts used to verify authenticated flows.
...
- Last result: direct Firebase Auth account creation succeeded against the active Firebase project using the canonical email/password, so this credential is now available for unattended authenticated QA.
...
4. Reflect the change in `docs/BLACKBOARD.md` if it affects unattended QA continuity.
...
- A canonical reusable QA account now exists for unattended protected-flow verification on the active Firebase project.
- Next unattended QA step should use this credential to run Playwright/browser-driven verification for `E-01`, then continue sequentially if the matrix clears.
