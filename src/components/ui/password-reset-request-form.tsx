...
export function PasswordResetRequestForm({ returnToLoginHref }: PasswordResetRequestFormProps) {
  ...
  function resolveResetError(submissionError: unknown): string {
    ...
    if (errorCode === 'auth/user-not-found') return t('notFound');
    if (errorCode === 'auth/invalid-email') return t('invalidEmail');
    if (errorCode === 'auth/too-many-requests') return t('tooManyRequests');
    if (errorCode === 'auth/network-request-failed') return t('networkError');
    ...
    <div className="flex min-h-screen w-full flex-col lg:flex-row bg-background-light font-display">
      ...
      <div className="relative hidden w-full lg:flex lg:w-1/2 flex-col justify-between p-12 overflow-hidden bg-primary/5">
        ...
        <div className="z-10 max-w-lg space-y-6">
          <h1 className="text-5xl font-extrabold tracking-tight text-slate-900" dangerouslySetInnerHTML={{ __html: t('title') }} />
      ...
      <main className="flex w-full flex-col justify-center items-center p-6 sm:p-12 lg:w-1/2">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2">
            <h2 className="text-3xl font-black tracking-tight text-slate-900">{t('formTitle')}</h2>
            <p className="text-slate-500">{t('formDesc')}</p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-8 shadow-xl">
            {status === 'success' ? (
              <div className="text-center">
                ...
                <h2 className="mb-2 text-xl font-bold text-slate-900">{t('statusSuccess')}</h2>
                <p className="mb-6 text-sm leading-relaxed text-slate-600">
                  {t('statusSuccessDesc', { email })}
                </p>
                <Link
                  href={returnToLoginHref as any}
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label htmlFor="email" className="mb-2 block text-sm font-bold text-slate-700">{t('emailLabel')}</label>
                  <input
                    id="email"
                    type="email"
                    value={email}
